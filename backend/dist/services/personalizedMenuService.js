...
exports.PersonalizedMenuService = void 0;
class PersonalizedMenuService {
     ...
     */
    generatePersonalizedSuggestions(menuItems, preferences, limit = 10) {
     ...
     */
    calculateItemScore(item, preferences, seasonalTrend) {
        let score = 0;
     ...
     */
    calculateSeasonalScore(item, seasonalTrend) {
        let score = 0;
        const itemTags = this.parseTags(item.tags);
        // Check if item tags match seasonal popular tags
        const matchingTags = itemTags.filter(tag => seasonalTrend.popularTags.includes(tag));
        ...
        const hasSeasonalIngredients = seasonalTrend.seasonalIngredients.some(ingredient => item.name.toLowerCase().includes(ingredient) ||
    ...
    calculatePreferenceScore(item, preferences) {
        ...
        if (preferences.cuisine) {
            const cuisineMatch = preferences.cuisine.some(cuisine => itemTags.includes(cuisine) || item.name.toLowerCase().includes(cuisine));
            score += cuisineMatch ? 10 : 0;
        }
        // General preferences matching
        if (preferences.preferences) {
            const preferenceMatches = preferences.preferences.filter(pref => itemTags.includes(pref) || item.name.toLowerCase().includes(pref));
     ...
     */
    calculateBudgetScore(item, budget) {
        if (!budget)
            return 0;
        const priceRanges = {
            low: { min: 0, max: 12 },
     ...
     */
    calculateDietaryScore(item, dietary) {
     ...
     */
    checkDietaryCompliance(item, requirement, itemTags) {
        const complianceMap = {
        ...
        if (!compliantTags)
            return true; // If we don't know the requirement, assume compliant
        return compliantTags.some(tag => itemTags.includes(tag));
    }
    /**
     * Match spice level preferences
     */
    matchSpiceLevel(item, preferredSpiceLevel) {
        const itemTags = this.parseTags(item.tags);
        const spiceLevels = {
            'mild': ['mild', 'gentle'],
        ...
        const preferredTags = spiceLevels[preferredSpiceLevel] || [];
        return preferredTags.some(tag => itemTags.includes(tag));
    }
    /**
     * Create a detailed menu suggestion
     */
    createMenuSuggestion(item, score, preferences, seasonalTrend) {
     ...
     */
    generateReasons(item, preferences, seasonalTrend) {
        const reasons = [];
        const itemTags = this.parseTags(item.tags);
        ...
        const feedback = this.getGuestFeedback(item.name);
        if (feedback) {
            const positiveFeedback = feedback.feedback.filter(f => !f.includes('too') && !f.includes('not'));
        ...
        // Preference reasons
        if (preferences.preferences) {
            const matchingPreferences = preferences.preferences.filter(pref => itemTags.includes(pref) || item.name.toLowerCase().includes(pref));
     ...
     */
    generateNotes(item, preferences) {
        const notes = [];
        const itemTags = this.parseTags(item.tags);
        // Price note
        if (item.price > 25) {
        ...
        if (preferences.dietary) {
            const compliant = preferences.dietary.filter(diet => this.checkDietaryCompliance(item, diet, itemTags));
    ...
    calculateMatchScore(item, preferences) {
        ...
        if (preferences.preferences) {
            const itemTags = this.parseTags(item.tags);
            const matches = preferences.preferences.filter(pref => itemTags.includes(pref) || item.name.toLowerCase().includes(pref));
     ...
     */
    getDietaryCompliance(item, dietary) {
        if (!dietary)
            return [];
        const itemTags = this.parseTags(item.tags);
        return dietary.filter(requirement => this.checkDietaryCompliance(item, requirement, itemTags));
    }
    /**
     * Get preference matches
     */
    getPreferenceMatches(item, preferences) {
        if (!preferences.preferences)
            return [];
        const itemTags = this.parseTags(item.tags);
        return preferences.preferences.filter(pref => itemTags.includes(pref) || item.name.toLowerCase().includes(pref));
    }
    /**
     * Generate summary statistics
     */
    generateSummary(suggestions, preferences) {
     ...
     */
    generateInsights(menuItems, seasonalTrend) {
     ...
     */
    getGuestFeedback(itemName) {
        return this.guestFeedbackData.find(feedback => feedback.itemName.toLowerCase() === itemName.toLowerCase());
    }
    /**
...
exports.PersonalizedMenuService = PersonalizedMenuService;
//# sourceMappingURL=personalizedMenuService.js.map