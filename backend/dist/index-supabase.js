...
const authenticateToken = async (req, res, next) => {
    ...
    try {
        ...
        const user = await supabase_1.dbHelpers.getUserById(decoded.userId);
        if (!user) {
...
app.post('/api/auth/login', async (req, res) => {
    try {
        ...
        const user = await supabase_1.dbHelpers.getUserByEmail(email);
        if (!user) {
...
app.get('/api/dashboard', authenticateToken, async (req, res) => {
    try {
        ...
        const dashboardData = await supabase_1.dbHelpers.getDashboardStats(userId);
...
app.get('/api/menus', authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        const menus = await supabase_1.dbHelpers.getMenusByUserId(userId);
        res.json({
            success: true,
...
app.post('/api/upload', authenticateToken, upload.single('menu'), async (req, res) => {
    try {
        ...
        const menu = await supabase_1.dbHelpers.createMenu(menuData);
        ...
        await supabase_1.dbHelpers.createUpload(uploadData);
        res.json({
            success: true,
...
app.delete('/api/menu/:id', authenticateToken, async (req, res) => {
    try {
        ...
        const menu = await supabase_1.dbHelpers.getMenuById(menuId);
        if (!menu || menu.user_id !== userId) {
            return res.status(404).json({ error: 'Menu not found' });
        }
        await supabase_1.dbHelpers.deleteMenu(menuId);
        res.json({
            success: true,
...
app.get('/api/user/settings', authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        const settings = await supabase_1.dbHelpers.getUserSettings(userId);
        if (!settings) {
...
app.post('/api/user/settings', authenticateToken, async (req, res) => {
    try {
        ...
        const existingSettings = await supabase_1.dbHelpers.getUserSettings(userId);
        if (existingSettings) {
            await supabase_1.dbHelpers.updateUserSettings(userId, settingsData);
        }
        else {
            await supabase_1.dbHelpers.createUserSettings(settingsData);
        }
        res.json({
            success: true,
...
startServer();
//# sourceMappingURL=index-supabase.js.map