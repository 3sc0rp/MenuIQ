...
const personalizedMenuService_1 = require("./services/personalizedMenuService");
...
app.post('/api/menu/upload', upload.single('menu'), async (req, res) => {
    try {
        ...
        const client = await pool.connect();
        const menuResult = await client.query('INSERT INTO menus (user_id, title) VALUES ($1, $2) RETURNING id', [userId || 1, title]);
        const menuId = menuResult.rows[0].id;
        // Insert menu items
        for (const item of menuItems) {
            await client.query('INSERT INTO menu_items (menu_id, name, price, cost, tags) VALUES ($1, $2, $3, $4, $5)', [
                menuId,
...
app.get('/api/menu/:id', async (req, res) => {
    try {
        ...
        const itemsResult = await client.query('SELECT * FROM menu_items WHERE menu_id = $1 ORDER BY created_at', [id]);
        client.release();
        res.json({
            menu,
...
app.get('/api/menu/:id/insights', async (req, res) => {
    try {
        ...
        const itemsResult = await client.query('SELECT * FROM menu_items WHERE menu_id = $1 ORDER BY sales_count DESC', [id]);
...
app.get('/api/menus', async (req, res) => {
    try {
        ...
        const client = await pool.connect();
        const result = await client.query('SELECT * FROM menus WHERE user_id = $1 ORDER BY created_at DESC', [userId || 1]);
        client.release();
        res.json({
            menus: result.rows
        });
...
app.post('/api/menu/:id/suggestions', async (req, res) => {
    try {
        ...
        const personalizedService = new personalizedMenuService_1.PersonalizedMenuService();
        // Generate personalized suggestions
        const suggestions = personalizedService.generatePersonalizedSuggestions(menuItems, preferences, limit);
        res.json({
            success: true,
            menuId: id,
            suggestions
