...
app.post('/api/menu/upload', authenticateToken, upload.single('menu'), async (req, res) => {
  try {
    ...
    // Insert menu
    const menuResult = await db.run(
      'INSERT INTO menus (user_id, title, file_name, items_count) VALUES (?, ?, ?, ?)',
      [userId, title || req.file.originalname, req.file.originalname, itemsCount]
    );
    ...
    if (menuItems.length > 0) {
      for (const item of menuItems) {
        await db.run(
          'INSERT INTO menu_items (menu_id, name, description, price, category, tags) VALUES (?, ?, ?, ?, ?, ?)',
          [
            menuId,
...
app.get('/api/menu/:id', authenticateToken, async (req, res) => {
  try {
    ...
    const items = await db.all(
      'SELECT * FROM menu_items WHERE menu_id = ? ORDER BY name',
      [id]
    );
...
async function generateInsights(menuId: number, items: any[]) {
  try {
    const insights = {
      ...
      categoryAnalysis: {} as Record<string, number>,
    ...
    // Store insights
    await db.run(
      'INSERT INTO insights (menu_id, insight_type, data) VALUES (?, ?, ?)',
      [menuId, 'comprehensive', JSON.stringify(insights)]
    );
...
app.get('/api/dashboard', authenticateToken, async (req, res) => {
  try {
    ...
    const menuStats = await db.get(
      'SELECT COUNT(*) as totalMenus, SUM(items_count) as totalItems FROM menus WHERE user_id = ?',
      [userId]
    );

    // Get recent menus
    const recentMenus = await db.all(
      'SELECT * FROM menus WHERE user_id = ? ORDER BY created_at DESC LIMIT 5',
      [userId]
    );
