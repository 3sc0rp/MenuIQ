...
export class MenuIQInsightEngine {
  ...
  private generateSuggestedAction(item: MenuItem, scores: any): { action: string; type: MenuIQScore['actionType'] } {
    ...
    const isSeasonal = tags.some((tag: string) => 
   ...
   */
  private determineSalesTrend(item: MenuItem): 'increasing' | 'stable' | 'declining' {
    const salesCount = item.sales_count || 0;
    
    // Simulate trend based on sales volume
    // In a real implementation, this would compare week-over-week data
    if (salesCount >= 50) return 'increasing';
    if (salesCount >= 20) return 'stable';
    return 'declining';
  }
   ...
   */
  private calculateProfitMarginScore(item: MenuItem): number {
    const margin = (item.price || 0) - (item.cost || 0);
    const marginPercentage = item.price > 0 ? (margin / item.price) * 100 : 0;
   ...
   */
  private calculateSalesVolumeScore(item: MenuItem): number {
    const salesCount = item.sales_count || 0;
    ...
    if (normalizedVolume >= 0.4) return 60;
    if (normalizedVolume >= 0.2) return 40;
    if (normalizedVolume >= 0.1) return 20;
    return 0;
  }
   ...
   */
  private calculateWasteRiskScore(item: MenuItem): number {
    const salesCount = item.sales_count || 0;
   ...
   */
  private calculateSeasonalityRiskScore(item: MenuItem): number {
    const tags = item.tags ? JSON.parse(item.tags) : [];
    const isSeasonal = tags.some((tag: string) => 
      tag.toLowerCase().includes('seasonal') ||
      tag.toLowerCase().includes('summer') ||
   ...
   */
  private getScoreCategory(score: number): MenuIQScore['category'] {
   ...
   */
  private generateInsights(item: MenuItem, scores: any): string[] {
   ...
   */
  private generateRecommendations(item: MenuItem, category: string, scores: any): string[] {
